...
class DongHoNuModel{
    loadspnu(vitribatdau){
        return new Promise(function (resolve, reject) {
            let sqlquerynu = `SELECT * FROM sanpham JOIN loaisanpham on sanpham.id_maloai = loaisanpham.maloai 
                              where loaisanpham.maloai = 2 and sanpham.isDelete = 0 and sanpham.trangthai = 1 limit ${vitribatdau}, 8`;
    ...
    loadAllSanPhamNu(){
        return new Promise(function (resolve, reject) {
            let sqlquerynu = `SELECT * FROM sanpham JOIN loaisanpham on sanpham.id_maloai = loaisanpham.maloai 
                              where loaisanpham.maloai = 2 and sanpham.isDelete = 0 and sanpham.trangthai = 1`;
            conn.query(sqlquerynu, function (err, result){
                if(err){
                    reject(err);
                }else{
